...
// ... Rest of your Admin/index.php code ...
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_user'])) {
        ...
        try {
            // Uses standard 'name' column mapped to your database schema layout
            $pdo->prepare("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)")->execute([$name, $email, $password, $role]);
            $new_user_id = $pdo->lastInsertId();

            // If added directly as merchant, seed their business profile row too
            if ($role === 'merchant') {
    ...
    // ADD MERCHANT ACTION (Promoting an existing user or creating new)
    ...
    if (isset($_POST['delete_target'])) {
        ...
        try {
            ...
            // 1. DELETE PROFILE: Wipe out their merchant business row details
            $stmt1 = $pdo->prepare("DELETE FROM merchants WHERE user_id = ?");
            $stmt1->execute([$user_id]);

            // 2. PURGE INVENTORY: Delete all products listed under this vendor's merchant_id
            $stmt2 = $pdo->prepare("DELETE FROM products WHERE merchant_id = ?");
            $stmt2->execute([$user_id]);

            // 3. REVERT ROLE & BAN: Set role to 'customer' AND flag merchant_status as 'revoked'
...
if ($active_tab === 'promos') {
    ...
    // 1. FETCH ALL USERS WHO HAVE THE MERCHANT ROLE FLAG IN THE SYSTEM (Safeguarded LEFT JOIN)
    try {
        $merchants = $pdo->query("
    ...
    // 2. FETCH ELIGIBLE USERS WHO ARE STILL STANDARD CUSTOMERS FOR THE DROPDOWN
    try {
    ...
    <main class="main-content">
        ...
        <section class="metrics-grid">
            <div class="metric-card">
                ...
                <div class="value">₹<?php echo number_format($total_revenue * 83, 0, '', ','); ?></div>
            </div>
            <div class="metric-card">
                    ...
                    <tbody>
                        <?php foreach($orders as $o): ?>
                            <tr>
                                ...
                                <td><strong>₹<?php echo number_format($o['total_price'] * 83, 0, '', ','); ?></strong></td>
       ...
       <!-- TAB: MERCHANTS SECTION -->
        <?php elseif ($active_tab === 'merchants'): ?>
            <div class="promo-split-container">
                ...
                <!-- Form Panel to Authorize Merchants -->
                <div class="data-board form-panel">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <form action="index.php?tab=merchants" method="POST" onsubmit="return confirm('Revoke merchant rights? User account defaults back to customer layer.');">
                                                ...
                                                <!-- Points securely to user_id to transition roles backwards -->
            ...
            <div class="promo-split-container">
                <div class="data-board form-panel">
                    ...
                    <form action="index.php?tab=promos" method="POST">
                        ...
                        <div class="form-group"><label>Coupon Code String</label><input type="text" name="code" class="form-control" placeholder="E.G. FESTIVE50" required></div>
                        <div class="form-group">
                        ...
                        <div class="form-group"><label>Expiration timeline Boundary</label><input type="date" name="expiry_date" class="form-control" required></div>
                        ...
                        <tbody>
                            <?php foreach($promos as $p): ?>
                                <tr>
                                    ...
                                    <td><strong><?php echo $p['discount_type'] === 'percentage' ? ((int)$p['discount_value'] . '%') : ('$' . $p['discount_value']); ?> OFF</strong></td>
